...
bool ViconBridge::init_vicon() {
  ...
  // setup unlabeled markers
  client_.EnableUnlabeledMarkerData();
  assert(client_.EnableUnlabeledMarkerData().Enabled);
...
void ViconBridge::create_unlabeled_markers_thread(int idx) {

  RCLCPP_INFO(get_logger(), "Creating new object %d", idx);

  boost::mutex::scoped_lock lock(segments_mutex_);
  SegmentPublisher &spub =
      segment_publishers_[tf_namespace_ + "/unlabeled_" + std::to_string(idx)];

  lock.unlock(); // we dont need the lock anymore

  // register the publisher
  spub.pub = this->create_publisher<geometry_msgs::msg::TransformStamped>(
      tf_namespace_ + "/unlabeled_" + std::to_string(idx), 10);

  // register the pose publisher
  spub.pub_poseMsg = this->create_publisher<geometry_msgs::msg::PoseStamped>(
      tf_namespace_ + "/unlabeled_" + std::to_string(idx) + "/pose", 10);

  spub.is_ready = true;
}

void ViconBridge::create_unlabeled_markers(int idx) {

  boost::thread(&ViconBridge::create_unlabeled_markers_thread, this, idx);
}
...
void ViconBridge::process_frame(rclcpp::Time &grab_time) {
  ...
  if (publish_specific_segment_) {
    ...
    process_all_segments_and_unlabled_markers(grab_time - latency);
  }
...
void ViconBridge::process_all_unlabeled_markers(const rclcpp::Time &frame_time) {

  std::size_t n_unlabeled_markers = client_.GetUnlabeledMarkerCount().MarkerCount;

  SegmentMap::iterator pub_it;

  std::vector<geometry_msgs::msg::TransformStamped> transforms;

  for (std::size_t i_unlabeled_marker = 0; i_unlabeled_marker < n_unlabeled_markers; ++i_unlabeled_marker) {

    geometry_msgs::msg::TransformStamped transform;
    bool success =
        get_transform_unlabeled_marker_msg(transform, frame_time, i_unlabeled_marker);
    ...
    if (success) {
      ...
      if (lock.owns_lock()) {
        pub_it = segment_publishers_.find(tf_namespace_ + "/unlabeled_" + std::to_string(i_unlabeled_marker));
        if (pub_it != segment_publishers_.end()) {
          ...
          lock.unlock();
          create_unlabeled_markers(i_unlabeled_marker);
        }
      }
...
void ViconBridge::process_all_segments_and_unlabled_markers(const rclcpp::Time &frame_time) {
...
bool ViconBridge::get_transform_unlabeled_marker_msg(geometry_msgs::msg::TransformStamped &msg,
                                    const rclcpp::Time &frame_time,
                                    int idx) {

  auto res_trans =
      client_.GetUnlabeledMarkerGlobalTranslation(idx);

  if (res_trans.Result != Result::Success) {
    RCLCPP_WARN(get_logger(), "getTransform failed for %d",
                idx);
    return false;
  }
  ...
  msg.header.stamp = frame_time;
  msg.child_frame_id =
      tf_namespace_ + "/unlabeled_" + std::to_string(idx);
